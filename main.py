...
# Objetivo: implementar fondos con paralax y animación de sprite (idle/run) con control básico.
import pygame, sys, math, random

pygame.init()
WIDTH, HEIGHT = 960, 540       
WINDOW = pygame.display.set_mode ((WIDTH, HEIGHT))
pygame.display.set_caption ("Práctica 4 - FFondos y Animaciones")
...
class StarLayer:
    ...
    def _init_(self, w, h, density=0.0008, speed=0.4):
        self.w, self.h= w, h
        self.speed = speed
        self.offset= 0.0
...
def make_run_frames(size=(48,48)):
    ...
    for i in range(8):
        ...
        phase = i/8*math.tau
        arm = int(6*math.sin(phase)%6)
        leg = int(6*math.cos(phase)%6)
        # brazos
...
# 2) Estado 'salto' (JUMP): agrega una animación y físca vertical simple (gravedad).
