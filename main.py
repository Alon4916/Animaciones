...
try:
    ...
    print("Pygame no está instalado. Instálalo con: pip install pygame==2.1.6")
    sys.exit(1)
...
def make_run_frames(size=(48,48)):
    ...
    for i in range(8):
        ...
        phase = i/8*math.tau
        arm = int(6*math.sin(phase))
        leg = int(6*math.cos(phase))
...
class AnimSprite(pygame.sprite.Sprite):
    def __init__(self, pos):
        ...
        self.special_mode = False  # NUEVO estado al presionar espacio
    ...
    def handle_input(self):
        ...
        # NUEVO: activar modo especial con ESPACIO
    ...
    def animate(self, dt):
        ...
        # NUEVO: cambiar color y cara si se presiona ESPACIO
        if self.special_mode:
            self.image = self.image.copy()
            self.image.fill((100, 180, 255), special_flags=pygame.BLEND_RGB_ADD)
            pygame.draw.circle(self.image, (0,0,0), (self.image.get_width()//2-6, 18), 2)
            pygame.draw.circle(self.image, (0,0,0), (self.image.get_width()//2+6, 18), 2)
            pygame.draw.rect(self.image, (0,0,0), (self.image.get_width()//2-6, 28, 12, 3), border_radius=1)
...
def main():
    ...
    hills = HillLayer(WIDTH, HEIGHT, color=(35, 90, 70), base=400, amp=36, freq=0.010, speed=2.0)
    ...
    while running:
        ...
        hud = [
            ...
            "Izq/Der o A/D para mover | ESPACIO cambia color/cara | ESC para salir",
        ]
